...
;  foray-success    ; Probability of finding a vacancy if foray, used in i-should-scout-direct
                    ; Moved to slider
...
;  set foray-success 0.07  ; Estimated from a simulation experiment
...
;   file-print count turtles-here
  ...
  if (scout-trait = "random") and (not I-should-scout-random) [stop]
 ; if (scout-trait = "always") Scouting will always be done if this trait is chosen
  if (scout-trait = "never") [stop]
  if (scout-trait = "indirect") and (not I-should-scout-indirect) [stop]
  if (scout-trait = "direct") and (not I-should-scout-direct) [stop]
...
to-report I-should-scout-direct  ; a turtle reporter, returns a boolean
  ...
  ; The method is derived in the instructor materials for Ch. 19.
  ...
  let time-horizon-survival-threshold 0.1  ; "Lifetime" is defined as number of years that bird
                                           ; has this probability of surviving until
  let time-horizon (ln time-horizon-survival-threshold / ln survival-prob) / 12
  ; show time-horizon
...
scout-trait
scout-trait
"random" "always" "never" "indirect" "direct"
4
...
@#$#@#$#@
# Woodhoopoe Model
This file is provided as instructor materials for Chapter 19 of _Agent-based and Individual-based Modeling, 2nd edition_, by Railsback and Grimm (2019). Please do not copy or distribute this file. It is available upon request from www.railsback-grimm-abm-book.com.

It was modified in March 2022 to correct an error in the `i-should-scout-direct` procedure.

This file is copyrighted 2019-2022 by Steven F. Railsback and Volker Grimm.

This is the simplified Woodhoopoe model from Section 19.4.3. The key adaptive trait of scouting for vacant territories is left unspecified for Exercise 2 of Chapter 19.
...
@#$#@#$#@
<experiments>
  <experiment name="CompareTraits" repetitions="20" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>year</metric>
    <metric>count turtles with [age-in-months &gt; 12]</metric>
    <metric>count patches with [count turtles-here with [is-alpha?] &lt; 2]</metric>
    <enumeratedValueSet variable="scout-prob">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="scout-trait">
      <value value="&quot;random&quot;"/>
      <value value="&quot;never&quot;"/>
      <value value="&quot;direct&quot;"/>
      <value value="&quot;direct-simple&quot;"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="ScoutProb" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>scout-attempts</metric>
    <metric>scout-successes</metric>
    <steppedValueSet variable="scout-prob" first="0" step="0.001" last="1"/>
    <enumeratedValueSet variable="scout-trait">
      <value value="&quot;random&quot;"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="Direct-ForaySuccess" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>scout-attempts</metric>
    <metric>scout-successes</metric>
    ...
    <steppedValueSet variable="foray-success" first="0.03" step="0.001" last="0.1"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
