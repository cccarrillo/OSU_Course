; 09Objects was written by Charles Ehlschlaeger 9/9/2012.
...
globals [
  ...
  countOfBare
  countOfTurtles
  turtleEnergyLossPerTimeStep
  grassGrownPerTimeStep
  weedsGrownPerTimeStep
]
...
to Setup
  ...
  set turtleEnergyLossPerTimeStep 6
  set grassGrownPerTimeStep 7.5
  set weedsGrownPerTimeStep 2.5
  ...
  [
    set color white
    set energy 50
    set size NewSize energy
    set sick false
  ]
  output-print "timeStep,GrassLand,WeededLand,BareLand,NumberTurtles"
  reset-ticks
end
...
to Go
  ...
  [ 
    ...
    ask patch-at 0 0
    [
      set foodPotential grass * 0.1
      set grass grass * 0.9 - 1.0
    ]
    set energy energy - turtleEnergyLossPerTimeStep + foodPotential
    set size NewSize energy
    ifelse energy < 25 and sick = false [
      set color yellow
    ][
      set color white
    ]
    ...
    if foodPotential < turtleEnergyLossPerTimeStep
    [
      fd 1 ;; turtles move forward one step, the width of one patch, if hungry
    ]
...
to AdjustGrass
  set grass grass + grassGrownPerTimeStep - random 4
  if grass < 0
  [
    set grass 0
  ]
...
to AdjustWeeds
  set weeds weeds + weedsGrownPerTimeStep - random 4
  if weeds < 0
  [
    set weeds 0
  ]
...
to-report NewSize [ turtleEnergy ]
  report .25 + turtleEnergy * .01
end
@#$#@#$#@
GRAPHICS-WINDOW
271
