...
to set-my-status
 ;dog age is incremented.
 ;each pack updates its alpha males and females
 ifelse age = 0
  [set status "pup"]
  [ifelse age = 1
    [set status "yearling"]
...
to update-pack-alphas
   ;if there is no alpha of a sex, a subordinate of that sex is randomly selected and its social status set to alpha
...
to-report logistic-function [x]
 ; based on the equations on page 222 of chapter 16
end

to reproduce
  ; if pack does not include both alpha female and male, no pups are produces
  ; if both alphas are present, probability of pack producing pups depents on logistic function. probability has a value of 0.5 when current time step, N, is half the carrying capacity
  ; and a value of 0.1 when N equals the carrying capacity
end
...
to do-pack-formation
  ...
  ; if pack has 2 or more subordinates of same sex, they always form a group
