globals [years-to-simulate prob-reproduction]
breed [dogs dog]
breed [packs pack]
...
to setup
  ...
  [
    ...
    [
      ...
      set age random 7
      set-my-status ; a dog procedure that sets social status from age
    ...
    ; select alpha dogs
    update-pack-alphas ; a pack procedure to give the pack 2 alphas
...
to go
  ...
  ; all dogs should be linked to a pack or disperser group and all collectives should be linked to dogs
  ...
  ask turtles with
  [any? link-neighbors with [breed = [breed] of myself]]
  [
...
to disperse ; a pack procedure
  ...
  ; first, identify the subordinates and stop if non
  let my-subordinates link-neighbors with
  [status = "subordinate"]
...
to create-disperser-group-from [some-dogs] ; a pack procedure
  ...
  [
    ...
    ; now, the disperser group set the variables of the dispersing dogs

    ask some-dogs
    [
      ; first we need to unlink to the pack
      ask my-links [die]

      ; next we need ot create a link with the disperser group where "myself" is now the disperser group
      create-link-with myself
      set status "disperser"
      set color green

      ; display on a radius from the disperser group
      move-to myself
      set heading [heading] of myself
      fd 1 + random-float 2
    ] ; end of ask some-dogs
  ] ; end of hatch-disperser-groups
end ; end of the to create-disperser-group-from
...
to reproduce
  if count link-neighbors with [status = "alpha"] = 2 ;this is asking if there is 1 male and 1 female alpha in the pack
  [
   let number-pups random-poisson 7.9 ;based on number of mean birth rate on page 220 of book
...
to do-mortality
  ; probability of dying is 0.44 for dispersers
  ; 0.25 for yearlings
  ; 0.2 for subordinates and alphas
  ; 0.12 for pups
...
to do-pack-mortality
  ; if has no memebers, it is removed
  ; if contains only pups, pups die and pack is removed
  ifelse count pack-members = 0 [die]
  [
    if all? pack-members [status = "pup"] [die]
  ]
end

to do-pack-formation
  ; if pack has no subordinate, no group is created
  ; if pack has only 1 subordinate of the same sex, it has a probability of 0.5 of forming a one-member group
  ; if pack has 2 or more subordinates of same sex, they always gorm a group
  ; dogs that join group need ot set their social staus to "disperser"

  let num-groups-met random-poisson count disperser-groups * 1
  let our-sex sex
  let our-pack [my-pack] of one-of natal-pack-ID
  let our-members natal-pack-ID
  let new-pack-formed false

  repeat num-groups-met
  [
    ask one-of disperser-groups
    [
      let their-members natal-pack-ID
      let their-sex sex
      let their-pack [my-pack] of one-of natal-pack-ID
      ...
      [
        ...
        [
          ...
          [
            set shape "circle"
            setxy random-xcor random-ycor
            set pack-members (turtle-set their-members our-members)
            ask pack-members
            [
              set shape "square"
              set heading random 360
